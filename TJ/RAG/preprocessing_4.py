...
def preprocess_dense_text_with_columns(row, column_names):
    ...
    for col_name, value in zip(column_names, row):
        ...
        if value.lower() in ['nan', 'null', '', 'none']:
            value = ""
        # 컬럼 이름과 값을 포함한 텍스트를 소문자로 변환
        row_data.append(f"{col_name.lower()}: {value.lower()}")
...
def save_to_json(output_dir, schema_name, file_name, sparse_chunks, dense_chunks, llm_chunks, df, file_path):
    ...
    for i, (sparse_text, dense_text, llm_text) in enumerate(zip(sparse_chunks, dense_chunks, llm_chunks)):
        # 청크 메타정보 추가
        sparse_text = f"[file_name: {file_name}, chunk_id: {i}]\n{sparse_text}"
        dense_text = f"[file_name: {file_name}, chunk_id: {i}]\n{dense_text}"
        llm_text = f"[file_name: {file_name}, chunk_id: {i}]\n{llm_text}"
...
if __name__ == "__main__":
    ...
    output_dir = "./processed6_json"  # JSON 파일을 저장할 디렉토리
