...
# Dense Text 전처리 함수
def preprocess_dense_text(text):
    text = str(text).strip()  # 공백 제거
    text = re.sub(r'^\s*$', '', text)  # 빈 줄 제거
    text = re.sub(r'\s+', ' ', text)  # 연속 공백 제거
    text = text.replace('nan', '')  # NaN 값을 제거
    return text
...
# LLM Text 전처리는 원본 데이터를 그대로 사용
def preprocess_llm_text(text):
...
def csv_to_chunks(input_csv, max_chunk_length=400):
    ...
    for _, row in df.iterrows():
        ...
        row_dense = preprocess_dense_text("| " + " | ".join(map(str, row.values)) + " |")
        row_llm = preprocess_llm_text("| " + " | ".join(map(str, row.values)) + " |")
...
if __name__ == "__main__":
    ...
    output_dir = "./processed_json"  # JSON 파일을 저장할 디렉토리
