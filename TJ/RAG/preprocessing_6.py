...
def csv_to_chunks_with_korean(input_csv, column_name_dict, max_chunk_length=400):
    ...
    Sparse Text에 한글 컬럼명을 포함합니다.
    """
    # CSV 읽기
    df = pd.read_csv(input_csv, encoding="utf-8")
    column_names = df.columns.tolist()

    # Sparse, Dense, LLM 텍스트 청크 초기화
    ...
    for _, row in df.iterrows():
        # 한글 컬럼명을 기반으로 Sparse Text 생성
        row_data = [f"{column_name_dict.get(col_name, col_name)}: {str(value).strip()}" 
                    for col_name, value in zip(column_names, row)]
        # 한글 컬럼명 포함된 Sparse Text 생성 후 전처리 적용
        row_sparse = preprocess_sparse_text("| " + " | ".join(row_data) + " |")

        # Dense Text 및 LLM Text 생성
        row_dense = preprocess_dense_text_with_columns_korean(row, column_names, column_name_dict)
        row_llm = preprocess_llm_text_with_columns_korean(row, column_names, column_name_dict)
...
if __name__ == "__main__":
    ...
    output_dir = "./processed_korean_2_json"
